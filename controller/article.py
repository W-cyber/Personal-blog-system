...
def read(articleid):
    ...
    if  payed is False:
        ...
        temp = content[0:int(len(content) *0.5)]
        position = temp.index('</p>') + 4
    ...
    comment_list = Comment().get_comment_user_list(article, 0, 10)
    count = Comment().get_count_by_article(articleid)
    # commentid=Comment().find_commentid(commentid)
    total = math.ceil(count/10)
    return render_template('article-user.html', article=dict, position=position, payed=payed,
                           is_favorited=is_favorited, prev_next=prev_next, comment_list=comment_list, total=total,content_all=content_all)
