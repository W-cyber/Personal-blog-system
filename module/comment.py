...
class Comment(DBase):
    ...
    def insert_comment(self, articleid, content, ipaddr):
        ...
        comment = Comment(userid=session.get('userid'),articleid=articleid,
                  content=content, ipaddr=ipaddr, createtime=now, updatetime=now, hidden=0, replyid=0)
    ...
    def insert_reply(self, articleid, commentid, content, ipaddr):
        ...
        comment = Comment(userid=session.get('userid'), articleid=articleid,
                          ...
                          createtime=now, updatetime=now, hidden=0)
        dbsession.add(comment)
        dbsession.commit()
    ...
    #个人中心
    #评论
    def find_my_comment(self):
        comment = dbsession.query(Comment,Article).join(Article, Comment.articleid ==Article.articleid).filter(Comment.userid == session.get('userid'),Comment.hidden==0).all()
        return comment
    def find_all_comment(self):
        comment = dbsession.query(Comment,Article).join(Article, Comment.articleid ==Article.articleid).filter(Comment.hidden==0).all()
        return comment
    def switch_hidden(self, commentid):
    ...
    # def find_commentid(self,commentid):
    #     result=dbsession.query(Comment).filter(Comment.commentid==commentid,Comment.hidden==0).all()
    #     return result
