...
class Article(DBase):
    ...
    def find_most_9(self):
        ...
        return result
        # 特别推荐文章：如果超过9篇，随机显示 ：order by rand()
    ...
    def switch_checked(self, articleid):
        ...
        return row.checked
    #根据userid查询出草稿文章
    def find_drafted_by_userid(self,userid):
        result=dbsession.query(Article).filter_by(userid=userid,drafted=1,hidden=0).all()
        return result
    #按用户id查询文章
    def find_by_userid(self,userid):
        result=dbsession.query(Article).filter_by(userid=userid,hidden=0,drafted=0,checked=1).all()
        return result
    #根据articleid获取到草稿文章的headline和content
    def find_drafted_by_articleid(self,articleid):
        result=dbsession.query(Article.headline,Article.content).filter_by(articleid=articleid,hidden=0,drafted=1).all()
        return result
